...
class CrunchApi(private val baseUrl: String = BASE_URL) {
    ...
    private val jsonMediaType = "application/x-www-form-urlencoded".toMediaType()

    fun login(username: String, password: String): Result<LoginResponse> {
        return try {
            val formBody = "login=$username&password=$password"
            val request = Request.Builder()
                .url("$baseUrl$LOGIN_PATH")
                .post(formBody.toRequestBody(jsonMediaType))
                .build()
    ...
    fun getBarcode(credentials: UserCredentials): Result<BarcodeResponse> {
        return try {
            ...
            if (!response.isSuccessful) {
                return Result.failure(IOException("Barcode fetch failed: ${response.code} $body"))
            }

            val json = JSONObject(body)
            Result.success(BarcodeResponse.fromJson(json))
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
    ...
    fun getGooglePayBarcodeJwt(credentials: UserCredentials): Result<String> {
        return try {
            ...
            if (!response.isSuccessful) {
                return Result.failure(IOException("Google Pay barcode fetch failed: ${response.code} $body"))
            }

            val jwt = body.trim().removeSurrounding("\"")
            Result.success(jwt)
        } catch (e: Exception) {
            Result.failure(e)
        }
